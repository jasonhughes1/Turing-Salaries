...
const Card = (props) => {
  ...
  const formattedGrad = moment(props.gradDate.slice(0, -14)).format('MMM. d, YYYY')
  const formattedCohort = moment(props.originalStartDate.slice(0, -14)).format('MMM. d, YYYY')
  return (
    <div className='card'>
      ...
      <p><span className="card-titles">Offer Accepted:</span> {formatted}</p>
      <p><span className="card-titles">Days to Offer:</span> {props.daysToOffer}</p>
      <p><span className="card-titles">Graduation Date:</span> {formattedGrad}</p>
      ...
      <p><span className="card-titles">Turing Start Date:</span> {formattedCohort}</p>
    </div>
  )
}
